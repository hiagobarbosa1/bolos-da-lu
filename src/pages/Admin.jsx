...
export default function Admin() {
  ...
  return <div className="admin"><aside><a href="#inicio">♥ Bolos <i>da Lu</i></a><small>PAINEL ADMINISTRATIVO</small>{['Visão geral', 'Pedidos', 'Produtos', 'Pronta entrega', 'Galeria de bolos', 'Avalia\u00e7\u00f5es', 'Clientes', 'Relatórios'].map((nome) => <button className={aba === nome ? 'ativo' : ''} onClick={() => setAba(nome)} key={nome}>{nome}</button>)}</aside><section><header><div><small>Olá, Lu! ✦</small><h1>{aba}</h1></div><a href="#inicio">Ver loja ↗</a></header>{erro && <p className="erro">{erro}</p>}
    ...
    {aba === 'Pedidos' && <><p className="dica-arrastar">⋮⋮ Segure a alça pontilhada e solte o pedido na coluna desejada.</p><div className="quadro">{etapas.map(([status, titulo]) => <div className={`coluna ${colunaDestino === status ? 'coluna-destino' : ''}`} key={status} onPointerEnter={() => { if (pedidoArrastado) setColunaDestino(status) }} onPointerLeave={() => { if (pedidoArrastado && colunaDestino === status) setColunaDestino(null) }}><h2>{titulo} <span>{total(status)}</span></h2>{pedidos.filter((p) => p.status === status).map((p) => { const prazo = prazoEntregaPedido(p.criado_em); return <article className={pedidoArrastado === p.id ? 'arrastando' : ''} key={p.id}><span className="alca-arraste" onPointerDown={(e) => { e.preventDefault(); setPedidoArrastado(p.id); setColunaDestino(null) }} title="Arrastar pedido" aria-label="Arrastar pedido">⋮⋮</span><b>#{p.id} · {p.usuarios?.nome || 'Cliente'}</b><small>{p.itens_pedido?.[0]?.nome_produto || p.itens_pedido?.[0]?.produtos?.nome || 'Encomenda personalizada'}</small><small>Feito: {prazo.realizado}</small>{p.endereco?.includes('agendada:') ? <small>{p.endereco}</small> : <small>Entrega: {prazo.estimativa}</small>}<small>Pagamento: {p.forma_pagamento || 'Não informado'}{p.forma_pagamento === 'Pix' && (p.pagamento_confirmado_em ? ' · Recebido' : ' · Pendente')}</small>{p.forma_pagamento === 'Pix' && !p.pagamento_confirmado_em && <button className="confirmar-pix-admin" disabled={confirmandoPix !== null} onClick={() => confirmarPix(p.id)}>{confirmandoPix === p.id ? 'Confirmando...' : 'Confirmar recebimento do Pix'}</button>}{!p.endereco?.includes('agendada:') && <ContagemRegressiva fim={prazo.fim} agora={agora} status={p.status}/>}<select value={p.status} onChange={(e) => trocarStatus(p.id, e.target.value)}>{etapas.map(([v, n]) => <option value={v} key={v}>{n}</option>)}</select></article> })}</div>)}</div></>}
...
function ContagemRegressiva({ fim, agora, status }) { if (status === 'entregue' || status === 'cancelado') return <span className="tempo-pedido finalizado">Pedido finalizado</span>; const diferenca = fim.getTime() - agora; const atrasado = diferenca < 0; const totalSegundos = Math.floor(Math.abs(diferenca) / 1000); const horas = Math.floor(totalSegundos / 3600); const minutos = Math.floor((totalSegundos % 3600) / 60); const segundos = totalSegundos % 60; const tempo = `${horas ? `${horas}h ` : ''}${String(minutos).padStart(2, '0')}m ${String(segundos).padStart(2, '0')}s`; return <span className={`tempo-pedido ${atrasado ? 'atrasado' : 'no-prazo'}`}>{atrasado ? `Atrasado há ${tempo}` : `Restam ${tempo}`}</span> }
